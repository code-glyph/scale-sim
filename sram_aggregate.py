...
def aggregate_data(df, config_name):
    aggregations = {
        'Avg IFMAP SRAM BW': 'mean',
        'Avg FILTER SRAM BW': 'mean',
        'Avg OFMAP SRAM BW': 'mean',
        'Avg IFMAP DRAM BW': 'mean',
        'Avg FILTER DRAM BW': 'mean',
        'Avg OFMAP DRAM BW': 'mean'
    }
...
for directory in glob.glob(os.path.join(base_dir, pattern)):
    compute_report_path = os.path.join(directory, 'BANDWIDTH_REPORT.csv')
    ...
    if os.path.exists(compute_report_path):
        ...
        aggregate_path = os.path.join(directory, 'Aggregate_BW.csv')
        aggregated_df.to_csv(aggregate_path)
        print(f"Created Aggregate_BW.csv in {directory}")
    else:
        print(f"BANDWIDTH_REPORT.csv not found in {directory}")
